...
describe('setVariantBaseSalePrice', () => {
  it('updates the target variant cost + re-bakes member_price from the stored fee', async () => {
    ...
    const res = await setVariantBaseSalePrice('b1', 'var_aaaaaaaaaaaa', 400);
    expect(res.ok).toBe(true);
    ...
    expect(v?.nexus_cost).toBe(400);
    expect(v?.member_price).toBe(410); // 400 + 10% of (500-400)
    expect(offer?.nexusFeePct).toBe(10); // fee % untouched
