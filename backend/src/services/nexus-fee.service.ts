...
/**
 * PLATFORM-ADMIN edit of one variant's ORIGINAL sale price (nexus_cost) from the
 * price popover - equivalent to the owner changing it in the Edit Offer modal:
 * the variant's member_price re-bakes from the offer's fee %, the representative
 * mirror + displayPrice recompute, and adopter overrides re-sync with the SAME
 * visibility split as updateOffer (tenant_only SNAPS the displayed price to the
 * new base; ecosystem preserves adopter margins and only clamps).
 *
 * Input: offerId, variantId, salePrice (> 0, must not exceed the variant's
 *        face_value - the relaxed rule allows equality).
 * Output: the updated offer, or a typed failure reason.
 */
