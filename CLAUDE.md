...
- **Collections** (`models/payments/wallet-payments.models.ts`, both in account-deletion): `walletPaymentCards` (PayMe `buyer_key` token + mask/brand/expiry; buyerKey NEVER serialized to clients) and `walletPurchases` (one doc per attempt; unique partial index `uniq_active_purchase_per_variant` on `(identityId, offerId, variantId)` where `active: true` = the "max 1 unit per variant per user" rule, DB-enforced; `active` is $unset on failure so retries work, KEPT on refund).
- **Purchase flow** (`services/wallet/purchase.service.ts`): card ownership check -> server-side price via `purchase-pricing.helper.ts` (mirrors catalog pricing exactly: adopter override, owner base, ecosystem base; shekels -> agorot) -> insert pending (dup key -> `already_purchased`) -> CLAIM a `voucherCodes` unit `available->assigned` BEFORE charging (none -> `out_of_stock`) -> `paymeChargeToken` (failure -> unit released + `card_declined`) -> completed + fire-and-forget SUMIT receipt (`purchase-receipt.service.ts`, outcome on `purchase.receipt`, never fails the purchase). Installments: `PURCHASE_INSTALLMENTS = 1` (single seam for the future).
- **Routes**: `POST/GET /api/v1/wallet/purchases[/mine]` + `GET /api/v1/wallet/purchases/:purchaseId/receipt` (PDF proxy) in `wallet-purchases.routes.ts`; saved cards `GET/POST/DELETE /api/v1/wallet/payment-cards` in `wallet-payment-cards.routes.ts`; PayMe IPN `POST /api/v1/payments/payme/callback` in `payme-callback.routes.ts` (public, urlencoded, ALWAYS 200; reconciliation matches purchaseId + payme_sale_id + exact price and ignores everything else). The old mock `purchase.routes.ts` stub was REMOVED.
