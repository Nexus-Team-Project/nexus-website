...
- **Collections** (`models/payments/wallet-payments.models.ts`): `walletPaymentCards` (PayMe `buyer_key` token + mask/brand/expiry; buyerKey NEVER serialized to clients) and `walletPurchases` (one doc per attempt carrying `quantity` units 1..`PURCHASE_MAX_QUANTITY`; only lookup indexes - the old `uniq_active_purchase_per_variant` unique index + `active` flag were REMOVED with multi-quantity, superseded by the per-customer cap below). **Account deletion (2026-07-23): cards + balances are hard-deleted, but `walletPurchases` are RETAINED as tenant/tax audit records** - every purchase doc snapshots `buyerName`/`buyerEmail` at purchase time, and `deleteMongoUser` backfills the snapshot on older docs + stamps `buyerDeletedAt` instead of deleting (dry-run count key: `walletPurchasesRetained`). Rationale: bookkeeping/audit retention (SUMIT receipts already carry the name and must be kept ~7 years); purchases are purpose-limited to audit/accounting after deletion.
- **Purchase flow** (`services/wallet/purchase.service.ts`): card ownership check -> server-side price via `purchase-pricing.helper.ts` - **FULL-FACE-VALUE CHARGE + CASHBACK (2026-07-22): the buyer is CHARGED the variant's full `face_value` (a 500 voucher costs 500; a variant with no face value falls back to the sale price), and the gap down to the DISPLAYED sale price (adopter override, owner base, ecosystem base - mirrors catalog pricing exactly) is stored per unit as `WalletPurchase.cashbackAgorot` and CREDITED to the buyer's Nexus balance (`creditPurchaseCashback` in `balance.service.ts` - best-effort, never fails the purchase) when the purchase completes; the credit is guarded by the atomic pending->completed transition so the sync charge path and the IPN `sale-complete` fallback never double-credit. `PurchaseView.cashbackAgorot` exposes the per-unit credit (0 on pre-cashback purchases); the wallet totals the offer page at face value and keeps the cashback line, and the purchase mutation invalidates `WalletBalance`. Refund clawback is NOT implemented (refund policy is still a pending management decision)** -> insert pending -> **PER-CUSTOMER CAP (2026-07-22, `purchase-quantity.helper.ts`): a customer may hold at most `PURCHASE_MAX_QUANTITY`=5 units of one variant CUMULATIVE across their pending+completed purchases (refunded/failed free the allowance); enforced insert-then-recount so concurrent attempts see each other's pending docs - violation -> purchase marked failed + `quantity_limit` (409), nothing charged. The wallet mirrors the cap on the stepper (owned units subtracted) and localizes the error** -> CLAIM `quantity` `voucherCodes` units `available->assigned` BEFORE charging (not enough -> `out_of_stock`) -> `paymeChargeToken` (failure -> units released + `card_declined`) -> completed + fire-and-forget SUMIT receipt (`purchase-receipt.service.ts`, outcome on `purchase.receipt`, never fails the purchase). Inventory/collection helpers live in `purchase-inventory.helper.ts` (shared with the IPN handler). Installments: `PURCHASE_INSTALLMENTS = 1` (single seam for the future).
...
- **Google One Tap silent login + Monday lead + partner cashback (2026-07-23, spec `../docs/superpowers/specs/2026-07-23-google-one-tap-silent-login-cashback-design.md`):** logged-out visitors on the homepage (`HomeContent`) + `/partners` get Google One Tap via `src/hooks/useGoogleOneTap.ts` (GIS script, prompt only when auth restore finished + no user + no silent flag + `VITE_GOOGLE_CLIENT_ID` set; every origin must be an Authorized JavaScript origin on the Google OAuth client). The credential POSTs the EXISTING `/api/auth/google` with `source:'one_tap'` + `page`; `AuthContext.oneTapLogin` stores the session but sets `localStorage['nexus_one_tap_silent']` FIRST, so the site keeps its logged-out look: `GeoDetectHome`/`WebsiteHomeSessionGate` skip the dashboard handoff, `Navbar` renders the guest layout with the sign-in link relabeled "Continue"/"המשך" (flag helpers in `src/lib/oneTapSilent.ts`; cleared on Login/Signup mount, navbar auth-link clicks, `redirectDashboardUser`/`redirectWebsiteSessionToDashboard`, and logout - non-silent sessions behave exactly as before). Backend: `googleAuth` also returns `email`/`fullName`; the route fires `createOneTapLead` (fire-and-forget, `monday-lead.service.ts` third producer) ONLY when `shouldFireOneTapLead` = one_tap idToken + `isNew` (user creation is the dedup); `ONE_TAP_LEAD_PRODUCTION_ONLY = true` - production-only (was false during the 2026-07-23 dev verification). Cashback: `Partner.cashbackPct Float?` (migration `20260723000000_add_partner_cashback_pct`), populated by `backend/scripts/import-partner-cashback.ts` (dry-run default, `--apply`; percent parser + name normalizer in `scripts/import-partner-cashback/parse.ts`, manual overrides in `mapping.data.ts`; 159 partners populated 2026-07-23, deliberate skips documented in the mapping file header incl. the כלל ביטוח 3-row conflict), stripped for guests by `stripGuestPartnerFields` in `partners.route.ts`, rendered as a violet badge on `PartnerCard` + a guest hero hint on `PartnersPage`.
