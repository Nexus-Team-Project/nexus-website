...
- **Wallet email+password auth (2026-07-14):** the nexusWallet signs in with the SAME Prisma `User.passwordHash` via `POST /api/v1/auth/password/{login,verify,resend}` + `/forgot/{start,verify,complete}` (`routes/wallet-password.routes.ts` -> `services/auth/wallet-password.service.ts`). One flow: correct password OR unknown email (auto-signup; bcrypt of the new password stashed on the challenge) both return `{ mode: '2fa_required', challengeToken }`; a 6-digit email code is REQUIRED on EVERY wallet password login (no trusted-device cookie). Challenges reuse Mongo `loginOtpChallenges` with new optional `purpose` (`wallet_login|wallet_signup|wallet_reset`) + `pendingPasswordHash` + `consumedAt` fields (`prismaUserId` is now nullable); purpose-routed email templates in `services/email/wallet-password-email.service.ts`. Per-account lockout: 5 FAILED attempts / 15 min per email (Mongo bucket `pwd_fail` via `countRecentEvents`/`recordEvent` in `wallet-rate-limit.ts`). Forgot is code-based (decoy token for unknown emails - zero enumeration), works for PASSWORDLESS accounts (sets a first password), rejects the current password (`password_unchanged`), and revokes ALL refresh tokens. **Password policy now ENFORCED** (8-128 + digit + upper/lower + special, `utils/password-policy.ts`) on website register + reset and wallet signup + reset; website `forgotPassword` also allows passwordless accounts now. Spec: `docs/superpowers/specs/2026-07-14-wallet-email-password-auth-design.md`.
